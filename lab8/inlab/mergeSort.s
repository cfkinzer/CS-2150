...
; Return type is void 
mergeSort:
	cmp rsi, rdx		; compare start and end
	jge end			; if start >= end, goto end
	push rsi		; store start
	push rdx		; store end
	add rdx, rsi		; start+end
	shr rdx, 1		; start+end/2
	push rdx		; store mid
	call mergeSort		; first half recursion
	pop rsi			; mid goes to param 2
	pop rdx			; end goes to param 3
	inc rsi			; mid+1 in param 2
	push rdx		; store end
	call mergeSort		; second half recursion
	pop rdx			; end goes back
	pop rsi			; start goes back
	mov rcx, rdx		; move end to param 4
	add rdx, rsi 		; start+end
	shr rdx, 1		; start+end/2 = mid
	call merge		; merge that bitch
end:
	ret			; ret
